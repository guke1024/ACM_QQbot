...
class PTEZOJ(Contest):
    async def get_contest(self):
        with open('./oj_json/ptezoj_contests.json', 'r', encoding='utf-8') as f:
    ...
    async def get_recent_info(self):
        ...
        return "PTEZOJ比赛还有15分钟就开始啦，没有报名的尽快报名~\n" + recent

    async def format_ptezoj_contest(self, next_contest, durationSeconds):
        res = "比赛名称：{}\n" \
              "开始时间：{}\n" \
              "持续时间：{}\n" \
              "比赛地址：{}\n".format(
